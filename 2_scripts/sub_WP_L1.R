sub_WP_L1 <- function(pasta_L1) {
  ...
  source("2_scripts/le_planilha_L1.R")
  ...
  lista_L1[[2]] <- lista_L1[[2]] %>%
    left_join(WP_sub, by = c(saida = "saida", WP_I = "WP")) %>%
    dplyr::select(c(1:5, "datahora_I" = 9, "lng_I" = 10, "lat_I" = 11, 6:7)) %>%
    left_join(WP_sub, by = c(saida = "saida", WP_F = "WP")) %>%
    dplyr::select(c(1:9, "datahora_F" = 12, "lng_F" = 13, "lat_F" = 14, 10))
  ...
  lista_L1[[4]] <- lista_L1[[4]] %>%
    left_join(WP_sub, by = c(saida = "saida", WP_I = "WP")) %>%
    dplyr::select(c(1:5, "datahora_I" = 20, "lng_I" = 21, "lat_I" = 22, 6:18)) %>%
    left_join(WP_sub, by = c(saida = "saida", WP_F = "WP")) %>%
    dplyr::select(c(1:9, "datahora_F" = 23, "lng_F" = 24, "lat_F" = 25, 10:21))
  
  lista_L1[[5]] <- lista_L1[[5]] %>%
    left_join(WP_sub, by = c(saida = "saida", WP_I = "WP")) %>%
    dplyr::select(c(1:3, "datahora_I" = 7, "lng_I" = 8, "lat_I" = 9, 4, 5)) %>%
    left_join(WP_sub, by = c(saida = "saida", WP_F = "WP")) %>%
    dplyr::select(c(1:7, "datahora_F" = 10, "lng_F" = 11, "lat_F" = 12, 8))
